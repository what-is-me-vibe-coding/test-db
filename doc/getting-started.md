...
| `widb_http_requests_total` | Counter | HTTP 请求总数（标签 endpoint: /query /write /health /admin/flush /admin/compact /other；method: GET POST；status: 2xx 3xx 4xx 5xx） |
| `widb_http_request_duration_seconds` | Histogram | HTTP 请求耗时分布（标签 endpoint/method） |
