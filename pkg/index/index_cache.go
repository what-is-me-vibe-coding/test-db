...
const (
	// defaultIndexCacheCapacity 是 IndexCache 的默认容量（字节）。
	defaultIndexCacheCapacity = 32 * 1024 * 1024 // 32MB
)

// IndexCache 缓存索引数据（布隆过滤器、稀疏索引统计信息），
...
// IndexCacheConfig 是 IndexCache 的配置参数。
type IndexCacheConfig struct {
	BloomCapacity  int // 布隆过滤器缓存容量（字节），0 表示使用默认值
	SparseCapacity int // 稀疏索引缓存容量（字节），0 表示使用默认值
}

// NewIndexCache 创建一个 IndexCache 实例。
func NewIndexCache(cfg IndexCacheConfig) *IndexCache {
	bloomCap := cfg.BloomCapacity
	if bloomCap <= 0 {
		bloomCap = defaultIndexCacheCapacity
	}
	sparseCap := cfg.SparseCapacity
	if sparseCap <= 0 {
		sparseCap = defaultIndexCacheCapacity
	}
	return &IndexCache{
...
func (ic *IndexCache) GetBloom(segID uint64) (*bloom.BloomFilter, bool) {
...
// PutBloom 将布隆过滤器放入缓存。
func (ic *IndexCache) PutBloom(segID uint64, filter *bloom.BloomFilter) {
	if filter == nil {
		return
	}
...
func (ic *IndexCache) InvalidateBloom(segID uint64) {
	ic.bloomCache.Delete(bloomCacheKey(segID))
}

// GetSparse 从缓存中获取稀疏索引统计信息。
func (ic *IndexCache) GetSparse(segID uint64, colID uint32) (ColumnSparseStat, bool) {
...
func (ic *IndexCache) PutSparse(segID uint64, colID uint32, stat ColumnSparseStat) {
	size := 64 // 估算：两个 Value + 元数据
	ic.sparseCache.Put(sparseCacheKey(segID, colID), stat, size)
}

// InvalidateSparse 使指定 Segment 的所有稀疏索引缓存失效。
func (ic *IndexCache) InvalidateSparse(segID uint64, columnCount int) {
...
// IndexCacheStats 是 IndexCache 的统计信息。
