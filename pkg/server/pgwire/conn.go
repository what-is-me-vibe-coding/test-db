...
type connHandler struct {
	...
	// extMu 保护 extStmts / extPortals / extErr 的并发访问。
	// 这些字段用于 extended query 协议（Parse/Bind/Execute）的状态跟踪。
	extMu      sync.Mutex
	extStmts   map[string]*extendedStmt
	extPortals map[string]*extendedPortal
	extErr     error
}
...
// simple query 与 extended query 走不同路径，分别由 handleQuery 与
// dispatchExtended 处理。
func (h *connHandler) dispatchMessage(msg pgproto3.FrontendMessage) bool {
	...
	default:
		// 其它消息统一交由 extended query 协议分发（包括 Parse/Bind/
		// Describe/Execute/Close/Sync 等）。
		if !h.dispatchExtended(msg) {
			// 未识别的消息：忽略
			_ = m
		}
	}
...
// handleQuery 执行 SQL 查询并发送结果（simple query 路径）。
