...
func TestEngineWriteAndGet(t *testing.T) {
	...
	if v, exists := row.Columns["name"]; !exists || v.Str != "alice" {
		t.Errorf("expected name=alice, got %v", v)
	}
	if v, exists := row.Columns["age"]; !exists || v.Int64 != 30 {
...
func TestEngineFlushMultiple(t *testing.T) {
	...
	_ = eng.Write("k1", map[string]common.Value{"val": common.NewInt64(1)})
	...
	_ = eng.Write("k2", map[string]common.Value{"val": common.NewInt64(2)})
...
func TestEngineAutoRotate(t *testing.T) {
	...
	_ = eng.Write("k1", map[string]common.Value{
		"val": common.NewString("hello world this is a long string to trigger rotation"),
	})
