...
// 命令或 SQL → 调用 OnLine。返回 (nil, false) 表示继续；返回非 nil 错误
// 表示 IO 失败，调用方应终止循环。返回特殊值 nil + 回调要求退出时
// 通过 shouldExit=true 由外层 RunWithLiner 检测循环是否还要继续。
//
// 由于 Go 不支持多返回值同时表达「继续/退出」与「错误」两种语义，本函数
// 把 shouldExit 编码为 error：io.EOF 表示"用户主动 Ctrl-D 正常退出"，
// 自定义哨兵 errTTYExit 表示"OnLine 要求退出"，其他非 nil 表示 IO 错误。
// 但为了避免引入额外导出符号，简化方案是：返回 error 表达「IO 错误」，
// 通过 wrapper 类型 ttySignal 表达"应该退出"或"应该继续"。
...
//   - shouldExit=true → 返回 ttyExitSignal{}（RunWithLiner 识别为终止）
